...
function FacturesPage() {
  ...
  const load = async () => {
    setLoading(true);
    const [{data:f},{data:c},{data:jj}] = await Promise.all([
              ...
              <TableBody>
                {justificatifsVente.length === 0 ? (
                  ...
                  const lie = j.bon_commande_id
                    ? justificatifsVente.find((x: any) => x.id === j.bon_commande_id)
                    : j.devis_id
                    ? justificatifsVente.find((x: any) => x.id === j.devis_id)
                    : null;
                  return (
                    <TableRow key={j.id}>
                      <TableCell>
