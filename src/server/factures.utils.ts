...
- LIGNES DE DÉTAIL: sauf mention explicite contraire (ex: "P.U. TTC"), le Prix Unitaire et le Total de chaque ligne sont toujours en Hors Taxes (HT) — utilise ces valeurs directement dans prix_unitaire_ht et total_ht
  ...
  · BALAYAGE COMPLET DES PRIX : parcours TOUT le document et relève CHAQUE champ monétaire — prix unitaires, totaux de ligne (colonne "Total"/"Montant"), sous-totaux, remises, bloc totaux du bas (Total HT, TVA, Total TTC, Net à payer). Ces valeurs se contrôlent les unes les autres : ne te fie jamais au seul libellé d'une colonne.
  · CONTRÔLE HT/TTC PAR LE CALCUL (même SANS libellé « TTC ») : avant de figer le prix unitaire, VÉRIFIE-le. Calcule S = Σ(quantité × prix unitaire) sur toutes les lignes de détail, puis compare S au bloc totaux :
    → si S ≈ Total HT → les prix unitaires sont bien HT (prix_unitaire_ht) ;
    → si S ≈ Total TTC et S ≠ Total HT → les prix unitaires affichés sont en réalité TTC (même sans le mot « TTC ») : mets-les dans prix_unitaire_ttc.
    Recoupe avec la colonne Total de ligne : si Total_ligne ≈ quantité × PU, les deux colonnes sont de même nature et basculent ensemble ; si Total_ligne ≈ quantité × PU × (1 + taux), alors le PU est HT et le total est TTC.
    Le bloc totaux (Total HT, TVA, Total TTC) fait FOI sur la nature HT/TTC des lignes. En cas de doute persistant, garde le PU en HT.
    Reporte dans total_ht le total de ligne tel qu'il est IMPRIMÉ, dans la même base que le prix unitaire que tu as retenu. N'invente aucun nombre : n'utilise que les valeurs réellement lues.
